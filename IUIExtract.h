...
private:
    // Applies the named theme to the extract UI
    void ApplyTheme(const QString & krqstrThemeName);

    // Returns a known theme name, falling back to the current system theme
    QString NormaliseThemeName(const QString & krqstrThemeName) const;

    // Returns Dark or Light based on the current system palette
...
public:
    ...
    // Theme accessors
    QString GetThemeName() const                     {return m_qstrThemeName;}
    void SetTheme(const QString & krqstrThemeName);
