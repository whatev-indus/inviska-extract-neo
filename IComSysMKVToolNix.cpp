...
{
    // Prefer app bundles because command-line copies in /usr/local/bin may be older.
    QStringList qstrlApplicationsDirs;
    qstrlApplicationsDirs << "/Applications" << QDir::homePath() + "/Applications";
    qstrlApplicationsDirs.append(QStandardPaths::standardLocations(QStandardPaths::ApplicationsLocation));
    qstrlApplicationsDirs.removeDuplicates();

    QFileInfo qfiNewestMKVToolNixApp;
    QVersionNumber qvnHighestVersion;
    for (const QString & krqstrApplicationsDir : qstrlApplicationsDirs)
    {
        QDir qdirAppDirReader(krqstrApplicationsDir);
        if (qdirAppDirReader.exists() == false)
            continue;

        qdirAppDirReader.setFilter(QDir::Dirs | QDir::NoDotAndDotDot);
        qdirAppDirReader.setSorting(QDir::Name | QDir::Reversed);
        QFileInfoList qfilDirList = qdirAppDirReader.entryInfoList(QStringList() << "MKVToolNix*.app");

        for (const QFileInfo & krqfiAppBundle : qfilDirList)
        {
            if (krqfiAppBundle.fileName() == "MKVToolNix.app" && qfiNewestMKVToolNixApp.exists() == false)
                qfiNewestMKVToolNixApp = krqfiAppBundle;

            if (krqfiAppBundle.fileName().startsWith("MKVToolNix-"))
            {
                QString qstrVersion = krqfiAppBundle.completeBaseName();
                qstrVersion.remove("MKVToolNix-");
                QVersionNumber qvnCurrentVersion = QVersionNumber::fromString(qstrVersion);
                if (qfiNewestMKVToolNixApp.exists() == false || qvnCurrentVersion > qvnHighestVersion)
                {
                    qvnHighestVersion = qvnCurrentVersion;
                    qfiNewestMKVToolNixApp = krqfiAppBundle;
                }
            }
        }
    ...
    if (qfiNewestMKVToolNixApp.exists())
    {
        m_qstrMKVToolNixPath = qfiNewestMKVToolNixApp.absoluteFilePath() + "/Contents/MacOS";
        if (ValidateMKVToolNixPath(m_qstrMKVToolNixPath) == true)
        {
            SaveMKVToolNixLocation();
            return true;
