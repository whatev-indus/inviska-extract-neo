...
void IComUIMainWinBase::ReadSettings()
{
    ...
    if (bUseSpecifiedSize == false && width() < DFLT_WIN_WIDTH)
        resize(DFLT_WIN_WIDTH, height());
    ...
    setMinimumWidth(DFLT_WIN_WIDTH);
...
bool IComUIMainWinBase::IsWindowOnScreen(const QRect & krqrctWinRect)
{
    ...
    QRect qrctDesktop;
    for (QScreen* pqscScreen : QGuiApplication::screens())
        qrctDesktop = qrctDesktop.united(pqscScreen->geometry());
